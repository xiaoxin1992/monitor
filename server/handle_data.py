...
def data_flush_database(data):
    ...
    insert_host_sql = """insert into monitor_host_info(id,ip,host_info,alive)  values(%s,'%s','%s',%s)""" % (data['id'],data['host'],data['host_info'],1)
    update_host_sql = """update monitor_host_info set alive=1 where id=%s and ip='%s'""" % (data['id'],data['host'])
    mysql = my_db.mysql()
    mysql.connect()
...
def handle(data):
    ...
    for x in data:
        if x['id'] is config_data['id']:
            data_flush_database({'id':x['id'],'host':config_data['host'],'host_info':x['host_info'],'cmd_name':x['cmd_name'],'data':x['data'],'code':x['code'],'time':now_time})
